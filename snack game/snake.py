...
#game loop (run until the user asks to quit)
while True:
    
    for event in pygame.event.get():
        #if the user click in the window close button
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
        #the others touch of the keybord
    ...
    for black in snake_body[1:]:
        if head_pos[0] == black[0] and head_pos[1] == black[1]:
            init_vars()
